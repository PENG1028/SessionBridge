...
| 节点 | **Node** | 一台主机 | 可连接的计算单元（本机 / 远程 Agent / SSH） |
...
```
Browser/Web UI           Relay Server (VPS)           Agent Node (leaf)
┌──────────┐    WS      ┌──────────────────┐    WS   ┌─────────────┐
│ app/     │◀────────▶│ src/             │◀──────▶│ NodeRuntime │
│ page.tsx │           │ relay-server.ts  │         │  ├─ Shell   │
│ Terminal │           │  ├─ HTTP :8080   │         │  ├─ Claude  │
│ Claude   │           │  ├─ WS server    │         │  └─ 扩展    │
│ Sidebars │           │  ├─ InstanceMgr  │         └─────────────┘
└──────────┘           │  └─ EventBus     │
                        └──────────────────┘
```
...
```
一个 Node 可以运行多个 Instance
一个 Instance 绑定一个 Workspace
一个 Instance 使用一个 Plugin（adapter）
一个 Plugin 贡献一个 View（渲染在 Stage）
一个 Plugin 声明多个 Panel（渲染在侧栏）
Stage 显示一个 Scene（绑定到当前激活的 Instance）
```
