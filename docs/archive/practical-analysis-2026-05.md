...
```
你在家 → 打开本地面板（或浏览器访问 relay 的 Web UI）→ 操作远程节点上的 Claude Code
```
...
**能跑通吗：** ❌ 无 HTTP API
**现状：**
- relay 有 WebSocket 协议 + REST API，但缺少为 CI 设计的专用 HTTP 端点
...
> 状态：P0/P1 全部解决，P2 大部分解决，P3 部分解决。剩余未完成项见 ../architecture.md 的当前状态与路线。
